...
function http_post_with_retry(url, retry, reqBody, accessToken)
    ...
    local function cb(content_, code_, response_, request_)
        if timeout_occurred == nil then
            ...
            if code == 200 or code == 204 or code == 201 then
                module:log("debug", "External call was successful, content %s",
                           content_);
                content = content_
            else
                module:log("warn",
                           "POST request error  on public key request: Code %s, Content %s",
                           code_, content_);
                content = code;
            end
            done();
            ...
            module:log("warn",
                       "External call reply delivered after timeout from: %s",
                       url);
        end
    end
    ...
    local function cancel()
        ...
        if code == nil then
            ...
            module:log("warn",
                       "Timeout %s seconds making the external call to: %s",
                       http_timeout, url);
            ...
            if retry == nil then
                module:log("debug",
                           "External call failed and retry policy is not set");
                done();
...
function authenticate_octopus()
    ...
    local response = http_post_with_retry(octopusBaseUrl ..
                                              'auth/client/refresh-token', 1,
                                          credJson);
    ...
    if response == 401 or response == 403 or response == 400 then
        ...
        response = http_post_with_retry(octopusBaseUrl .. 'auth/client/login',
                                        1, credJson);
        ...
        if err == nil then
            module:log("info",
                       "New tokens saved successfully using refresh token");
        end
    end
    ...
    if response == 401 then
        ...
        if response == 404 or response == 400 then
            ...
            session.send(st.error_reply(stanza, "cancel", "not-allowed",
                                        "Room modification disabled for guests"));
            return true;
        else
            module:log("info",
                       "[verify user joining meeting] successfully sent",
                       response);
        end
    end
end
