...
import {
  ...
  getMultipleMeetingsByTenantIdAction,
...
const MeetingsByTenant: FC = () => {
  ...
  const {
    getMultipleMeetingsByTenantId: { meetingsByTenantId, loading },
  ...
  useEffect(() => {
    if (!deleteLoading || !createLoading)
      dispatch(
        getMultipleMeetingsByTenantIdAction(Number.parseInt(tenantId, 10))
      );
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        meetingsByTenantId?.map((meeting) => (
          <Card
            flex={false}
            width={{ max: '1000px' }}
...
export default MeetingsByTenant;
