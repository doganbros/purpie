...
export function completeDirectChannelDisplayName(
  ...
  if (isDirectChannel(channel)) {
    ...
    return Object.assign(dmChannelClone, {
      display_name: displayUsername(profiles[teammateId], teammateNameDisplay),
    });
  }
  if (
...
export function sortChannelsByTypeListAndDisplayName(
  ...
  if (aDisplayName !== bDisplayName) {
    return aDisplayName
      .toLowerCase()
      .localeCompare(bDisplayName.toLowerCase(), locale, { numeric: true });
  }
...
export function sortChannelsByTypeAndDisplayName(
  ...
  if (channelTypeOrder[a.type as any] !== channelTypeOrder[b.type as any]) {
    if (channelTypeOrder[a.type as any] < channelTypeOrder[b.type as any]) {
      return -1;
    }
...
export function sortChannelsByDisplayName(
  ...
  if (a.display_name && b.display_name && a.display_name !== b.display_name) {
    return a.display_name
      .toLowerCase()
      .localeCompare(b.display_name.toLowerCase(), locale, { numeric: true });
  }
...
export function filterChannelsMatchingTerm(
  ...
  return channels.filter((channel: Channel): boolean => {
    ...
    return (
      name.startsWith(lowercasedTerm) || displayName.startsWith(lowercasedTerm)
    );
  });
}
...
export function calculateUnreadCount(
  ...
  let mentions;
  if (crtEnabled) {
    messages = messageCount.root - member.msg_count;
