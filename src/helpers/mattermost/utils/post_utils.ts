...
export function shouldIgnorePost(
  ...
  const userIdCheck =
    post.props && post.props.addedUserId && post.props.addedUserId === userId;
...
export function canEditPost(
  ...
  canEdit = haveIChannelPermission(state, {
    channel: channelId,
    team: teamId,
    permission,
  });
...
export function isPostCommentMention({
  ...
  const notCurrentUser =
    post.user_id !== currentUser.id || (post.props && post.props.from_webhook);
  if (notCurrentUser) {
    if (
