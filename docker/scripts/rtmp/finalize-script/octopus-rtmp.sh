...
send_streaming_event(){
 ...
 echo "Sending Streaming Event: slug: $VIDEO_ID and userID is: $USER_ID URL: $OCTOPUS_URL" >> /tmp/octopus-rtmp.log
 ...
 echo "Response from Streaming Event: $RESPONSE" >> /tmp/octopus-rtmp.log
 if [[ $RESPONSE != OK ]]; then
 STATUS_CODE=$(echo ${RESPONSE} | jq -r '.statusCode')
 fi
...
send_recording_event(){
 HEADER="Bearer $AUTH_TOKEN"
 echo "Sending Recording Event: id: $FILE_ID and filename is: $FILENAME URL: $OCTOPUS_URL">> /tmp/octopus-rtmp.log
