...
FILENAME=$4
FINAL_STATUS_CODE=0
echo "#################################### $DATE --- Reporting Event to Octopus #########################################################" >> /dev/stdout
echo "1: $1 - 2: $2 - 3:$3 - 4: $4"  >> /dev/stdout
echo "$DATE --- Event Type: $EVENT_TYPE - User ID: $USER_ID - Video ID: $VIDEO_ID" >> /dev/stdout
...
upload() {
  ...
  if [[ $S3_EXIT_CODE == 0 ]] 
    then
    echo "/home/recordings/$FILE_ID successfully synced with $S3_BUCKET_NAME/meeting-recordings/$FOLDER_NAME/" >> /dev/stdout
    else
    echo "/home/recordings/$FILE_ID failed to sync with $S3_BUCKET_NAME/meeting-recordings/$FOLDER_NAME/. Status Code: $S3_EXIT_CODE" >> /dev/stdout
  fi
}
...
auth() {
  ...
  if [[ ${AUTH_STATUS_CODE} == 200 ]]; then
    echo "$DATE - Auth token successfully renewed" >> /dev/stdout
  elif [[ ${AUTH_STATUS_CODE} == 401 || ${AUTH_STATUS_CODE} == 403 || ${AUTH_STATUS_CODE} == 400 ]]; then
    echo "$DATE - Refresh Token has expired. Re-authing to Octopus..." >> /dev/stdout
    ...
    if [[ ${LOGIN_RETURN_CODE} == 200 ]]; then
      echo "$DATE - Successfully logged into Octopus" >> /dev/stdout
    else
      echo "$DATE - Error while re-auth. Returned: $LOGIN_RETURN_CODE"  >> /dev/stdout
    fi
  else
    echo "$DATE - Authentication failed with the status code: $AUTH_STATUS_CODE." >> /dev/stdout
  fi
  ...
  #Logging new tokens
  echo "$DATE - Auth Token: $AUTH_TOKEN" >> /dev/stdout
  echo "$DATE - Refresh Token: $REFRESH_TOKEN" >> /dev/stdout
}
...
send_event() {
  ...
  if [[ $EVENT_TYPE == record-done ]]
    then
    upload
    echo "$DATE - Sending Recording Event: id: $FILE_ID and filename is: $FILENAME" >> /dev/stdout
    ...
    echo "$DATE - Sending Streaming Event: slug: $VIDEO_ID and userID is: $USER_ID" >> /dev/stdout
  ...
  if [[ $RESPONSE == OK || $RESPONSE == Created ]]
   then
    echo "$DATE - Event successfully sent to Octopus with response: $SEND_EVENT_RETURN_CODE" >> /dev/stdout
  else
    if [[ $NUMBER_OF_TRIES -lt $MAX_EVENT_TRIES ]]
     then
      NUMBER_OF_TRIES=$((NUMBER_OF_TRIES+1))
      echo "$DATE - Error while sending event. Retuned code: ${SEND_EVENT_RETURN_CODE}. Num of tries: $NUMBER_OF_TRIES." >> /dev/stdout
      auth
      send_event      
    else
      echo "$DATE - Error while sending event after trying $NUMBER_OF_TRIES times. Retuned code: ${SEND_EVENT_RETURN_CODE}" >> /dev/stdout
    fi
  fi
}
